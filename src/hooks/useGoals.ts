...
export function calculateOddsScenarios(
  start: number,
  target: number,
  minOdds: number,
  maxOdds: number,
...
export function getKeyMetric(goal: Goal): {
  label: string;
  value: string;
  color: string;
} {
...
export function useGoals() {
    ...
    (async () => {
      try {
        ...
        if (apiGoals && apiGoals.length > 0) {
          ...
          // Index local goals by BOTH frontend id and backend id — the API returns
          // backend ids while local goals are keyed by their frontend id.
          const localMap = new Map<string, Goal>();
          for (const lg of localGoals) {
            localMap.set(lg.id, lg);
            const bid = (lg as { _backendId?: string })._backendId;
            if (bid) localMap.set(bid, lg);
          }
          const mapped = apiGoals.map((g) => {
            const bc = (g.config || {}) as Record<string, unknown>;
            const local =
              localMap.get(g.id) ||
              localMap.get(String(bc.id ?? "")) ||
              undefined;
            const base: Goal = {
              ...(bc as Record<string, unknown>),
              id:
                (local?.id as string | undefined) ||
                (bc.id as string | undefined) ||
                g.id,
              type: g.type as GoalType,
              name: (local?.name as string | undefined) || g.name,
              ...
              status: (local?.status ??
                (g.isCompleted ? "completed" : "active")) as GoalStatus,
              createdAt: (local?.createdAt as string | undefined) || "",
              // Local mutable state wins over stale backend config — isPrimary is only
              // persisted locally and never synced to the backend.
              ...(local
                ? {
                    isPrimary: local.isPrimary,
                    steps: local.steps,
                    currentBank: local.currentBank,
                    ladderMode: local.ladderMode,
                  }
                : {}),
              _backendId: g.id,
            };
  ...
  useEffect(() => {
    const snapshot = goals
      .map(
        (g) =>
          `${g.id}:${g.currentAmount ?? g.currentROI ?? g.currentWinRate ?? g.currentStep ?? 0}:${g.status}:${g.isPrimary ? 1 : 0}`,
      )
      .join(",");
    if (
  ...
  const createGoal = async () => {
    ...
    const goal: Goal = {
      ...
      betsPerDay: synced.betsPerDay,
    };
    if (synced.type === "amount") {
      goal.targetAmount = synced.targetAmount;
      goal.currentAmount = 0;
    } else if (synced.type === "ladder") {
      const steps = calculateLadderSteps(
        synced.startAmount,
        synced.targetLadderAmount,
        synced.minOdds,
        synced.maxOdds,
      );
      Object.assign(goal, {
        startAmount: synced.startAmount,
        targetLadderAmount: synced.targetLadderAmount,
        minOdds: synced.minOdds,
        maxOdds: synced.maxOdds,
        avgOdds: synced.minOdds,
        currentStep: 0,
        totalSteps: steps.length,
        ladderMode: synced.ladderMode,
        steps,
        currentBank: synced.startAmount,
      });
    } else if (synced.type === "roi") {
      goal.targetROI = synced.targetROI;
      goal.currentROI = 0;
    } else if (synced.type === "winrate") {
      goal.targetWinRate = synced.targetWinRate;
    ...
    UserDataService.createGoal({
      ...
      current: 0,
      isCompleted: false,
      config: goal as unknown as Record<string, unknown>,
    })
  ...
  const setPrimaryGoal = (goalId: string) => {
    ...
    bumpStrategy();
    toast.success(
      isPrimary ? "Головну ціль скасовано" : "Головна ціль змінена",
    );
  };
  ...
  const isLadderPreviewValid = (): boolean => {
    const start = safeParseFloat(startAmountStr),
      target = safeParseFloat(targetLadderAmountStr),
      min = safeParseFloat(minOddsStr),
      max = safeParseFloat(maxOddsStr);
    return (
      start > 0 &&
      target > 0 &&
      min > 1 &&
  ...
  const validateGoalNumeric = (
    data: ReturnType<typeof syncNewGoalFromStrings>,
  ): boolean => {
    if (
      data.type === "amount" &&
    ...
    if (data.type === "ladder") {
      if (!data.startAmount || data.startAmount <= 0) {
        toast.error("Початкова сума > 0");
        return false;
      }
      if (!data.targetLadderAmount || data.targetLadderAmount <= 0) {
        toast.error("Цільова сума > 0");
        return false;
      }
      if (data.startAmount >= data.targetLadderAmount) {
        toast.error("Ціль > початкової суми");
        return false;
      }
      if (!data.minOdds || data.minOdds < 1.01) {
        toast.error("Мін. коеф. ≥ 1.01");
        return false;
      }
      if (!data.maxOdds || data.maxOdds < 1.01) {
        toast.error("Макс. коеф. ≥ 1.01");
        return false;
      }
      if (data.minOdds >= data.maxOdds) {
        toast.error("Макс > Мін");
        return false;
      }
    }
    if (data.type === "roi" && (!data.targetROI || data.targetROI <= 0)) {
      toast.error("ROI > 0");
      return false;
    }
    if (
      data.type === "winrate" &&
      (!data.targetWinRate ||
        data.targetWinRate <= 0 ||
        data.targetWinRate > 100)
    ) {
  ...
  const getDisciplineStatus = (
    goal: Goal,
  ): { status: "good" | "warning"; label: string } => {
    ...
    const goalBets = betsData.filter(
      (bet: { goalId?: string }) => bet.goalId === goal.id,
    );
    ...
    return hasViolation
      ? { status: "warning", label: "Відхилення" }
      : { status: "good", label: "Дотримані" };
  };

  const containerStates = {
    isStepsCalculationExpanded,
    setIsStepsCalculationExpanded,
    isLadderOverviewExpanded,
    setIsLadderOverviewExpanded,
    isStepsProgressionExpanded,
    setIsStepsProgressionExpanded,
    isRulesExpanded,
    setIsRulesExpanded,
  };
  ...
  return {
    ...
    showCompletedResultModal,
    setShowCompletedResultModal,
    selectedGoal,
    setSelectedGoal,
    goalToDelete,
    isUpdating,
    ...
    startAmountStr,
    setStartAmountStr,
    targetLadderAmountStr,
    setTargetLadderAmountStr,
    targetAmountStr,
    setTargetAmountStr,
    targetROIStr,
    setTargetROIStr,
  ...
  if (goal.type === "ladder" && goal.steps && goal.minOdds && goal.maxOdds) {
    return {
      ...
      steps: goal.steps.map((s) =>
        !s.minPlannedAmount || !s.maxPlannedAmount
          ? {
              ...s,
              minPlannedAmount:
                Math.round(s.startAmount * (goal.minOdds || 1.3) * 100) / 100,
              maxPlannedAmount:
                Math.round(s.startAmount * (goal.maxOdds || 5) * 100) / 100,
            }
          : s,
      ),
...
function computeGoalProgress(goal: Goal, betsData: Bet[]): Goal {
  ...
  switch (goal.type) {
    ...
    case "roi": {
      const gb = betsData.filter(
        (b) => b.goalId === goal.id && b.result !== "Pending",
      );
...
function computeLadderProgress(goal: Goal, betsData: Bet[]): Goal {
  ...
  const goalBets = betsData.filter(
    (b) => b.goalId === goal.id && b.result === "Win",
  );
  const oddsBets = betsData.filter(
    (b) =>
      !b.goalId && b.result === "Win" && b.odds >= minOdds && b.odds <= maxOdds,
  );
  const allBets = [...goalBets, ...oddsBets].filter(
    (b, i, a) => a.findIndex((x) => x === b) === i,
  );
  ...
  let stepIdx = 0;
  const steps: LadderStep[] = (goal.steps || []).map((s, i) => ({
    ...s,
  ...
  for (let si = 0; si < steps.length; si++) {
    ...
    const minPlanned =
      cs.minPlannedAmount || Math.round(cs.startAmount * minOdds * 100) / 100;
    steps[si] = {
      ...cs,
      status: "completed",
      completedAt: bestBet.date,
      actualAmount: actual,
      actualOdds: bestBet.odds,
      deviation: actual - minPlanned,
    };
    stepIdx = si + 1;
    if (si + 1 < steps.length)
      steps[si + 1] = {
        ...steps[si + 1],
        startAmount: actual,
        minPlannedAmount: Math.round(actual * minOdds * 100) / 100,
        maxPlannedAmount: Math.round(actual * maxOdds * 100) / 100,
        status: "current",
      };
  }
  ...
  const remaining = calculateRemainingSteps(
    currentBank,
  ...
  if (stepIdx > 0 && stepIdx < steps.length && remaining > 0) {
    ...
    for (let i = stepIdx; i < totalNew; i++) {
      const minP = Math.round(running * minOdds * 100) / 100,
        maxP = Math.round(running * maxOdds * 100) / 100;
      steps.push({
        step: i + 1,
        startAmount: running,
        minPlannedAmount: minP,
        maxPlannedAmount: maxP,
        status: i === stepIdx ? "current" : "locked",
      });
