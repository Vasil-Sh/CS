...
import {
  getRiskColor,
  getRiskIcon,
  parseCriteriaForValidation,
} from "@/lib/strategyHelpers";
...
const STRATEGY_TEMPLATES: StrategyTemplate[] = [
  {
    ...
    description: "Безпечний підхід з низьким ризиком.",
    riskLevel: "Low",
    expectedROI: 8,
    criteria: [
  ...
  },
  {
    ...
    description: "Оптимальне співвідношення ризику та прибутку.",
    riskLevel: "Medium",
    expectedROI: 15,
    criteria: [
  ...
  },
  {
    ...
    description: "Високий ризик, високий прибуток.",
    riskLevel: "High",
    expectedROI: 25,
    criteria: [
...
export default function StrategyOverview({
  ...
  const [primaryStrategy, setPrimaryStrategy] = useState<string | null>(() => {
    ...
    const saved = UserDataService.getUserData<string>(
      currentUser,
      "primary_strategy",
      "",
    );
  ...
  const strategyMatchesPrimary = (s: CS2Strategy) => {
    if (!primaryStrategy) return false;
    return (
      primaryStrategy === (s.id || s.name) ||
      primaryStrategy === s._backendId ||
      primaryStrategy === s.name
    );
  };
  const getPrimaryStrategyName = () =>
    primaryStrategy
      ? strategies.find(strategyMatchesPrimary)?.name || primaryStrategy
      : null;
  const isTemplateAlreadyCreated = (t: string) =>
    strategies.some((s) => s.name.toLowerCase() === t.toLowerCase());
  ...
  const loadData = async () => {
    ...
    try {
      const j = await api.get<{ data?: unknown[] } | unknown[]>(
        "/bets?page=1&limit=200",
      );
      bets = (
        Array.isArray(j) ? j : (j as { data?: unknown[] }).data || []
      ) as typeof bets;
    } catch {
      bets = UserDataService.getUserData(
        currentUser,
        "mybets_data",
        [],
      ) as typeof bets;
    ...
    let strats = UserDataService.getUserData<CS2Strategy[]>(
      currentUser,
      "strategies_data",
      [],
    );
    try {
      ...
      if (apiStrats?.length) {
        strats = apiStrats.map((s) => {
          const cfg = (s.config || {}) as Record<string, unknown>;
          return {
            ...cfg,
            id: (cfg.id as string | undefined) || s.id,
            _backendId: s.id,
          } as unknown as CS2Strategy;
        });
        UserDataService.setUserDataSync(currentUser, "strategies_data", strats);
      }
    } catch {
      /* ignore */
    }
  ...
  const calcStats = (
    ...
    bets.forEach((b) => {
      ...
      stats[s].totalBets++;
      stats[s].totalStake += b.amount || 0;
      if (b.result === "Win") {
        stats[s].wins++;
        stats[s].totalProfit += b.profit || 0;
    ...
    Object.keys(stats).forEach((s) => {
      const c = stats[s].wins + stats[s].losses;
      stats[s].winRate = c > 0 ? (stats[s].wins / c) * 100 : 0;
      stats[s].roi =
        stats[s].totalStake > 0
          ? (stats[s].totalProfit / stats[s].totalStake) * 100
          : 0;
    });
  ...
  const saveStrategy = () => {
    if (!newStrategy.name || !newStrategy.description) {
      toast.error("Заповніть назву та опис");
      return;
    }
    ...
    if (
      strategies.some(
        (s) => s.name.toLowerCase() === newStrategy.name.toLowerCase().trim(),
      )
    ) {
      toast.error("Така стратегія вже існує");
      return;
    }
  ...
  const deleteStrategy = () => {
    ...
    const toRemove = strategies.find(
    ...
    UserDataService.deleteStrategy(
      toRemove?._backendId || strategyToDelete,
      toRemove?.name,
    ).catch(() => {});
  };
  ...
  const togglePrimaryStrategy = (strat: CS2Strategy) => {
    ...
    if (primaryStrategy === id) {
      setPrimaryStrategy(null);
      UserDataService.setUserDataSync(currentUser, "primary_strategy", "");
      store.setPrimaryStrategyId("");
      toast.success("Основну стратегію скасовано");
    } else {
      setPrimaryStrategy(id);
      UserDataService.setUserDataSync(currentUser, "primary_strategy", id);
      store.setPrimaryStrategyId(id);
      if (strat._backendId)
        UserDataService.setPrimaryStrategy(strat._backendId).catch(() => {});
      toast.success(`"${strat.name}" — основна стратегія`);
    }
    bumpStrategy();
  };
  ...
  const chartData = useMemo(() => {
    ...
    const mk = (fn: (s: StrategyStats) => number) =>
      entries
        .map(([name, stats]) => ({
          name: name.length > 15 ? name.substring(0, 15) + "..." : name,
          fullName: name,
          value: parseFloat(String(fn(stats))),
          totalBets: stats.totalBets,
          riskLevel:
            strategies.find((s) => s.name === name)?.riskLevel || "Medium",
        }))
        .sort((a, b) => b.value - a.value)
        .slice(0, 8);
    return {
      roiData: mk((s) => s.roi),
      winRateData: mk((s) => s.winRate),
      profitData: mk((s) => s.totalProfit),
    };
  ...
  const filteredStrategies = useMemo(
    () =>
      strategies
        ...
        .sort((a, b) => {
          const sa = strategyStats[a.name],
            sb = strategyStats[b.name];
          let cmp = 0;
          if (sortBy === "roi") cmp = (sa?.roi || 0) - (sb?.roi || 0);
          else if (sortBy === "profit")
            cmp = (sa?.totalProfit || 0) - (sb?.totalProfit || 0);
  ...
  return (
    <div className="space-y-6">
              ...
              <div className="bg-white/60 backdrop-blur-sm rounded-[32px] p-5 border-2 border-stone-200 shadow-[0_4px_16px_rgba(0,0,0,0.06)]">
                <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                  {filteredStrategies.map((s, i) => {
                    const stats =
                      strategyStats[s.name] || ({} as StrategyStats);
                    return (
                      <StrategyCard
                        key={s.id || s.name || i}
                        strategy={s}
                        stats={stats}
                        isPrimary={strategyMatchesPrimary(s)}
      ...
      {activeTab === "performance" && (
        <div className="space-y-6">
              ...
              recommendations={
                strategies.length > 0
                  ? [
                      {
                        type: "info" as const,
                        message: "Використовуйте стратегії з ROI > 5%",
                      },
                      {
                        type: "warning" as const,
                        message: "Уникайте ризикованих команд",
                      },
                      {
                        type: "success" as const,
                        message: "Ведіть статистику для покращення",
                      },
                    ]
                  : []
              }
          ...
          <StrategyPerformanceCharts
            roiData={chartData.roiData}
            winRateData={chartData.winRateData}
        ...
        >
          ...
          <div className="p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <Label className="text-gray-900 font-medium">Назва *</Label>
                <Input
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v: "Low" | "Medium" | "High") =>
                    setNewStrategy({ ...newStrategy, riskLevel: v })
                  }
                >
            ...
            <div>
              <Label className="text-gray-900 font-medium">Опис</Label>
              <Textarea
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <Label className="text-gray-900 font-medium">Критерії</Label>
                <Button
                  type="button"
                  variant="outline"
                  size="sm"
                  ...
                  <div key={i} className="flex gap-2">
                    <Input
                      ...
                      onChange={(e) =>
                        setNewStrategy({
                          ...newStrategy,
                          criteria: newStrategy.criteria.map((cc, ii) =>
                            ii === i ? e.target.value : cc,
                          ),
                        })
                      }
                    ...
                    {newStrategy.criteria.length > 1 && (
                      <Button
                        ...
                        onClick={() =>
                          setNewStrategy({
                            ...newStrategy,
                            criteria: newStrategy.criteria.filter(
                              (_, ii) => ii !== i,
                            ),
                          })
                        }
      ...
      <StrategyDetailsDialog
        ...
        stats={
          selectedStrategy ? strategyStats[selectedStrategy.name] : undefined
        }
        isPrimary={
          !!(selectedStrategy && strategyMatchesPrimary(selectedStrategy))
        }
      />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {STRATEGY_TEMPLATES.map((t, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`border rounded-2xl p-5 ${exists ? "bg-gray-50 border-gray-200 opacity-60 cursor-not-allowed" : "bg-white border-gray-200 hover:border-gray-300 cursor-pointer"}`}
                  onClick={() => {
                    if (!exists) {
                      setNewStrategy({
                        name: t.name,
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    <span className="flex items-center gap-2 font-semibold text-gray-900">
                      {getRiskIcon(t.riskLevel)}
                      <span className="text-sm">{t.name}</span>
                    </span>
                    <Badge
                      className={
                        getRiskColor(t.riskLevel) + " text-xs font-medium"
                      }
                    >
                      {t.riskLevel}
                    </Badge>
                  ...
                  <Button
                    className={`w-full rounded-xl font-medium mt-3 ${exists ? "bg-gray-300 text-gray-400 cursor-not-allowed hover:bg-gray-300" : "bg-gray-900 hover:bg-gray-800 text-white"}`}
                    size="sm"
                    disabled={exists}
                  >
                    {exists ? "Вже створено" : "Використати шаблон"}
                  </Button>
                </div>
              );
            })}
