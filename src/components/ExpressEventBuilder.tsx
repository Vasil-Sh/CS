...
export function ExpressEventBuilder({
  ...
  const maxMaps =
    format === "BO5" ? 5 : format === "BO3" ? 3 : format === "BO1" ? 1 : 3;
  ...
  const renderGroup = (group: {
    ...
    const safeValue = group.options.some((o) => o.value === modalBetType)
      ? modalBetType
      : undefined;
    const isGroupSelected =
      modalBetType && group.options.some((o) => o.value === modalBetType);
      ...
      return (
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-2">
            <Select
              value={safeValue}
              onValueChange={(v) => setModalBetType(v || "")}
            >
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={onClearAll}
            aria-label="Очистити всі події експресу"
      ...
      <div className="p-6 space-y-3">
        ...
        <div
          className={`p-4 rounded-2xl border ${
            expressRisk.color === "green"
              ? "bg-green-50 border-green-200"
              : expressRisk.color === "orange"
        ...
        >
          <div className="flex items-center justify-between mb-2">
            ...
            >
              {expressEvents.length}{" "}
              {expressEvents.length === 1
                ? "подія"
                : expressEvents.length < 5
                  ? "події"
                  : "подій"}
            </span>
          </div>
          ...
          >
            {expressRisk.color === "green" &&
              "Оптимальна кількість подій для контролю ризику"}
            {expressRisk.color === "orange" &&
              "Збільшений ризик через кількість подій. Рекомендуємо обережність."}
            {expressRisk.color === "red" &&
              "Дуже високий ризик! Кожна додаткова подія знижує ймовірність виграшу."}
          </p>
        </div>
        ...
        <div className="space-y-2 max-h-[500px] overflow-y-auto">
          {expressEvents.map((event, index) => {
            ...
            return (
              <div
                key={index}
                className="p-4 rounded-2xl border border-gray-200 bg-white flex flex-col gap-3"
                style={{
                  boxShadow:
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  >
                    {event.betType
                      ? getBetTypeLabel(event.betType, format)
                      : "Тип"}
                  </button>
                  <Select
                    value={event.selection}
                    onValueChange={(val) =>
                      onUpdateEvent(index, "selection", val)
                    }
                  >
                    <SelectTrigger
                      className={`rounded-xl bg-white h-9 text-xs ${needsSelection ? "border-blue-400 focus:border-blue-500" : "border-gray-200"}`}
                    >
                  ...
                  <Input
                    ...
                    value={event.odds}
                    onChange={(e) =>
                      onUpdateEvent(index, "odds", e.target.value)
                    }
                    placeholder="Коеф."
        ...
        {!allExpressEventsComplete && (
          <div className="p-3 bg-blue-50 rounded-2xl border border-blue-200">
            <p className="text-xs text-blue-800 flex items-center gap-1.5">
              ...
              Заповніть вибір та коефіцієнт для кожної події, щоб зберегти
              експрес
            </p>
          </div>
        )}
      ...
      >
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-gray-100">
            ...
            {modalTab >= 2 &&
              (
                grouped.maps.find((m) => m.mapNumber === modalTab - 1)
                  ?.groups ?? []
