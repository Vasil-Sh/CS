    ...
    >
      <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
        <div className="flex items-center gap-3">
          ...
          <span className="text-lg font-semibold text-gray-900">Останні записи</span>
          {activeBets.length > 0 && (
      ...
      <div className="border-b border-gray-100 bg-gray-50">
        <div className="flex items-center gap-3 px-6 py-3.5">
          ...
          <span className="text-sm text-gray-500 font-medium mr-1">Фільтр:</span>
          ...
          <button onClick={onToggleAdvancedFilters}
            ...
            Розширені
            {hasActiveAdvancedFilters && !showAdvancedFilters && <span className="flex items-center justify-center w-2 h-2 rounded-full bg-blue-500" />}
          </button>
          {hasActiveAdvancedFilters && (
            <button onClick={resetAdvancedFilters} className="px-3 py-2 rounded-xl text-sm font-medium text-red-500 bg-red-50 border border-red-200 hover:bg-red-100 transition-all duration-200">
              Скинути
            </button>
          )}
          {/* Search */}
          <div className="ml-auto flex items-center gap-2">
            <div className="relative">
              ...
              />
              {searchText && (
                <button onClick={() => onSearchTextChange('')} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-900">
      ...
      <div className="px-0 pb-6">
        {sortedAndFilteredBets.length > 0 ? (
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    <th className="text-center px-4 py-3.5 text-sm font-semibold text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition-colors"
                      onClick={() => toggleSort('date')}>
                      <div className="flex items-center justify-center gap-2">Дата<ArrowUpDown className="h-3 w-3" strokeWidth={1.5} /></div>
                    </th>
                    <th className="text-left px-4 py-3.5 text-sm font-semibold text-gray-500 uppercase tracking-wider min-w-[220px] border-l border-gray-200">Матч</th>
                    <th className="text-center px-4 py-3.5 text-sm font-semibold text-gray-500 uppercase tracking-wider border-l border-gray-200">Тип</th>
                    <th className="text-center px-4 py-3.5 text-sm font-semibold text-gray-500 uppercase tracking-wider border-l border-gray-200">Валюта</th>
                    <th className="text-center px-4 py-3.5 text-sm font-semibold text-gray-500 uppercase tracking-wider border-l border-gray-200">Сума</th>
                    <th className="text-center px-4 py-3.5 text-sm font-semibold text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition-colors border-l border-gray-200"
                    ...
                    <th className="text-center px-4 py-3.5 text-sm font-semibold text-gray-500 uppercase tracking-wider border-l border-gray-200">Ціль</th>
                    <th className="text-center px-4 py-3.5 text-sm font-semibold text-gray-500 uppercase tracking-wider border-l border-gray-200">Статус</th>
                    <th className="text-center px-3 py-3.5 text-sm font-semibold text-gray-500 uppercase tracking-wider border-l border-gray-200">Нотатки</th>
                    <th className="text-center px-4 py-3.5 text-sm font-semibold text-gray-500 uppercase tracking-wider border-l border-gray-200">Дії</th>
                  </tr>
                </thead>
                <tbody>
                  {paginatedBets.map((bet) => {
                    ...
                    return (
                      <tr key={betKey} className="border-b border-gray-100 hover:bg-gray-50 transition-colors duration-150">
                        <td className="px-4 py-4 text-center"><span className="text-base text-gray-700 font-medium">{bet.date}</span></td>
                        ...
                        <td className="px-4 py-4 text-center border-l border-gray-100">
                          {goalName ? (
                            <Badge className="font-medium px-2.5 py-1 rounded-md bg-blue-50 text-blue-500 border-0 text-sm max-w-[130px] truncate hover:bg-blue-50" title={goalName}>
                              <Flag className="h-3.5 w-3.5 mr-1.5 flex-shrink-0" strokeWidth={1.5} /><span className="truncate">{goalName}</span>
                            </Badge>
                          ) : <span className="text-gray-300 text-sm">—</span>}
                        </td>
                        <td className="px-4 py-4 text-center border-l border-gray-100">
                          <Badge className={`rounded-full border-0 font-semibold text-sm px-3.5 py-1.5 ${isWin ? 'bg-green-100 text-green-600 hover:bg-green-100' : isLoss ? 'bg-red-100 text-red-600 hover:bg-red-100' : 'bg-amber-100 text-amber-600 hover:bg-amber-100'}`}>
                        ...
                        <td className="px-4 py-4 text-center border-l border-gray-100">
                          <div className="flex gap-2 justify-center">
                            {isPending && (<>
                              <button onClick={() => onUpdateResult(bet, 'Win')} className="flex items-center justify-center w-9 h-9 rounded-xl border border-gray-200 hover:bg-green-100 hover:border-green-300 text-green-600 transition-all duration-200" title="Виграш"><CheckCircle className="h-4 w-4" strokeWidth={2} /></button>
                              <button onClick={() => onUpdateResult(bet, 'Loss')} className="flex items-center justify-center w-9 h-9 rounded-xl border border-gray-200 hover:bg-red-100 hover:border-red-300 text-red-600 transition-all duration-200" title="Програш"><XCircle className="h-4 w-4" strokeWidth={2} /></button>
                            </>)}
                            <button onClick={() => onShareBet(bet)} className="flex items-center justify-center w-9 h-9 rounded-xl border border-gray-200 hover:bg-blue-50 hover:border-blue-300 text-blue-500 transition-all duration-200" title="Поділитися"><Share2 className="h-4 w-4" strokeWidth={2} /></button>
                            {!isPending && (
                              <button onClick={() => onDeleteBet(bet)} className="flex items-center justify-center w-9 h-9 rounded-xl border border-gray-200 hover:bg-red-100 hover:border-red-300 text-red-600 transition-all duration-200" title="Видалити"><Trash2 className="h-4 w-4" strokeWidth={2} /></button>
                            )}
                            <button onClick={() => onBetDetails(bet)} className="flex items-center justify-center w-9 h-9 rounded-xl border border-gray-200 hover:bg-purple-50 hover:border-purple-300 text-purple-600 transition-all duration-200" title="Деталі"><Eye className="h-4 w-4" strokeWidth={2} /></button>
                          </div>
                        </td>
                      </tr>
                    );
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-1.5 pt-6 px-6">
                <button onClick={() => onPageChange(Math.max(1, currentPage - 1))} disabled={currentPage === 1}
                  className={`flex items-center justify-center w-9 h-9 rounded-xl transition-all duration-200 ${currentPage === 1 ? 'text-gray-300 cursor-not-allowed' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-900'}`} title="Попередня">
                  <ChevronLeft className="h-4 w-4" strokeWidth={2} />
                </button>
                {getPageNumbers().map((page, idx) => page === '...' ? (
                  <span key={`dots-${idx}`} className="flex items-center justify-center w-9 h-9 text-sm text-gray-400">…</span>
                ) : (
                ...
                <button onClick={() => onPageChange(Math.min(totalPages, currentPage + 1))} disabled={currentPage === totalPages}
                  className={`flex items-center justify-center w-9 h-9 rounded-xl transition-all duration-200 ${currentPage === totalPages ? 'text-gray-300 cursor-not-allowed' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-900'}`} title="Наступна">
          ...
          <div className="flex-1 flex items-center justify-center py-16 text-center">
            ...
            {tableFilter === 'today' ? (<>
              <h3 className="text-xl font-semibold text-gray-900 mb-2">Немає записів за сьогодні</h3>
              <p className="text-gray-500 text-sm mb-6">Додайте новий запис або перегляньте всі матчі</p>
              <Button onClick={() => onTableFilterChange('all')} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-blue-500 hover:bg-blue-600 text-white text-base font-semibold transition-colors">Показати всі матчі</Button>
            </>) : hasActiveAdvancedFilters ? (<>
              <h3 className="text-xl font-semibold text-gray-900 mb-2">Немає записів за обраними фільтрами</h3>
              <p className="text-gray-500 text-sm mb-6">Спробуйте змінити параметри фільтрації</p>
              <Button onClick={resetAdvancedFilters} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-blue-500 hover:bg-blue-600 text-white text-base font-semibold transition-colors">Скинути фільтри</Button>
            </>) : (<>
              <h3 className="text-xl font-semibold text-gray-900 mb-2">Поки що немає записів</h3>
              <p className="text-gray-500 text-sm mb-6">Додайте свій перший запис, щоб почати відстеження</p>
              <Button onClick={() => onNavigateToAdd?.()} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-blue-500 hover:bg-blue-600 text-white text-base font-semibold transition-colors">
