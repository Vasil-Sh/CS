...
const iconMap: Record<string, React.ReactNode> = {
  ...
  CheckCircle2: <CheckCircle2 className="h-10 w-10 text-primary" strokeWidth={1.5} />,
};
...
export default function OnboardingTour({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => { if (!open) finish(); }}>
      <DialogContent className="rounded-3xl max-w-lg border border-gray-200 p-0 gap-0" hideCloseButton>
        {/* Progress */}
        ...
        <div className="px-6 pt-6 pb-2 text-center space-y-4">
          <div className={`flex items-center justify-center w-20 h-20 rounded-3xl mx-auto ${ICON_COLORS[current.icon] || 'bg-gray-100'}`}>
            {iconMap[current.icon]}
          </div>
          <div>
            <DialogTitle className="text-xl font-bold text-gray-900 mb-2">{current.title}</DialogTitle>
        ...
        <DialogFooter className="px-6 pb-6 pt-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button onClick={() => step < STEPS.length - 1 ? setStep(s => s + 1) : finish()}
              className="rounded-xl bg-primary hover:bg-blue-700 text-white font-semibold text-sm px-5 h-10">
