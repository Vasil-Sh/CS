...
export default function TelegramGroups() {
  ...
  const resultBadge = (result: string) => {
    switch (result) {
      case 'Win': return <Badge className="bg-green-100 text-green-600 border border-green-200 text-xs font-medium rounded-full"><CheckCircle2 className="h-3 w-3 mr-1" strokeWidth={1.5} />Виграш</Badge>;
      ...
      <Dialog open={groupDialogOpen} onOpenChange={setGroupDialogOpen}>
        <DialogContent className="rounded-2xl max-w-md">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSaveGroup} className="rounded-xl bg-blue-500 hover:bg-blue-600">
  ...
  const renderKPICards = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
      <StatCard label="Груп" value={groups.length} icon={Users} color="bg-blue-50" iconColor="text-blue-500" />
      <StatCard label="Ставок" value={overallStats.totalBets} icon={BarChart3} color="bg-blue-50" iconColor="text-blue-500" />
      <StatCard 
        label="Win Rate" 
  ...
  return (
    <>
      ...
      {groups.length === 0 ? (
        <Card className="border-2 border-gray-300 rounded-2xl bg-white overflow-hidden flex-1 flex items-center justify-center" style={{ boxShadow: CHART_CARD_SHADOW }}>
                ...
                <div className="px-7 py-4">
                  <div className="flex items-center gap-3">
                    <div className="flex items-center justify-center w-11 h-11 rounded-2xl bg-blue-50 flex-shrink-0">
                      <ExternalLink className="h-5 w-5 text-blue-500" strokeWidth={1.5} />
                    </div>
                    <a href={g.link} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-500 hover:underline truncate flex-1">
                      {tgHandle(g.link)}
                    </a>
                    <a
                      ...
                      className="text-xs font-medium text-white bg-blue-500 hover:bg-blue-600 inline-flex items-center gap-1 px-3 py-1.5 rounded-lg transition-colors flex-shrink-0"
            ...
            <div className="space-y-4 flex-1 px-7 pb-7 pt-6">
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  className="flex-1 rounded-xl bg-blue-500 hover:bg-blue-600 text-white text-sm font-semibold transition-colors"
                >
                ...
                <button
                  ...
                  className="p-2 rounded-xl border border-red-100 hover:bg-red-50 text-red-500 hover:text-red-600 transition-colors flex-shrink-0"
...
function StabilityBadge({ stability, label }: { stability: number; label: string }) {
  ...
  const colors = stability >= 70
    ...
    : stability >= 40
    ? 'bg-yellow-50 text-yellow-600 border-orange-200'
    : label === 'Немає даних'
