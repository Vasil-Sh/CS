      ...
      <div className="p-5 flex flex-col flex-1">
        <div className="flex items-center justify-between mb-4">
          <span className="flex items-center gap-2">
            ...
            <span
              className="font-semibold text-[#111827] break-words"
              title={strategy.name}
            >
              {strategy.name}
            </span>
            {isPrimary && (
              <Badge className="bg-[#EFF6FF] text-[#3B82F6] border-0 rounded-full text-xs px-2 py-0.5 font-medium hover:bg-[#EFF6FF]">
                <Star
                  className="h-2.5 w-2.5 mr-0.5 fill-[#3B82F6]"
                  strokeWidth={1.5}
                />
                Основна
              </Badge>
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div className="flex flex-col items-center justify-center p-3 bg-white border border-[#E5E7EB] rounded-2xl">
            <div
              className={`text-2xl font-bold ${(stats.roi || 0) >= 0 ? "text-[#22C55E]" : "text-[#EF4444]"}`}
            >
              {(stats.roi || 0) >= 0 ? "+" : ""}
          ...
          <div className="flex flex-col items-center justify-center p-3 bg-white border border-[#E5E7EB] rounded-2xl">
            <div
              className={`text-2xl font-bold ${(stats.winRate || 0) >= 50 ? "text-[#3B82F6]" : "text-[#6B7280]"}`}
            >
        ...
        <div className="flex gap-2 mt-auto">
          ...
          >
            <Star
              className={`h-4 w-4 ${isPrimary ? "fill-[#3B82F6]" : ""}`}
              strokeWidth={1.5}
            />
          </Button>
          <Button
