    ...
    <div className="min-h-screen bg-[#f3f3f3] relative">
      ...
      <div className="relative z-10 space-y-8 px-6 lg:px-8 pb-8 pt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <StatCard icon={<Trophy className="h-5 w-5 text-[#111827]" strokeWidth={1.5} />} label="Виграші" value={String(winningBets.length)} valueColor="text-[#22C55E]" subtext="Успішних записів" />
          <StatCard icon={<AlertTriangle className="h-5 w-5 text-[#111827]" strokeWidth={1.5} />} label="Програші" value={String(losingBets.length)} valueColor="text-[#EF4444]" subtext="Невдалих записів" />
          <StatCard icon={<TrendingUp className="h-5 w-5 text-[#111827]" strokeWidth={1.5} />} label="Середній ROI" value={`${stats.averageROI >= 0 ? '+' : ''}${stats.averageROI}%`} valueColor={stats.averageROI >= 0 ? 'text-[#111827]' : 'text-[#EF4444]'} subtext={stats.averageROI >= 0 ? 'Позитивний' : 'Негативний'} subIcon={stats.averageROI >= 0 ? <ArrowUpRight className="h-4 w-4 text-[#22C55E]" strokeWidth={2.5} /> : <ArrowDownRight className="h-4 w-4 text-[#EF4444]" strokeWidth={2.5} />} />
        </div>
