...
export default function LoginPage({ demo = false }: LoginPageProps) {
  ...
  const isAdminInput = !demo && username.toLowerCase() === 'admin';
  ...
  const seoCanonical = demo ? 'https://matchiq.pro/login-digesto-demo' : 'https://matchiq.pro/login';

  const headingText = demo ? 'Digesto Демо' : isAdminInput ? 'Вхід адміністратора' : 'Вітаємо знову';
  const subText = demo ? 'Увійдіть до демо-облікового запису' : 'Увійдіть до свого облікового запису';
  const footerLinkText = demo ? 'Не демо?' : 'Потрібна допомога?';
  const footerLinkHref = demo ? '/login' : '#';
  const footerLinkLabel = demo ? 'Повна версія входу' : "Зв'яжіться з підтримкою";
    ...
    <div className="min-h-screen relative overflow-hidden bg-[#F5F5F0]">
      ...
      <div className="relative z-10 flex items-center justify-center min-h-screen p-6">
        <Card className={`w-full max-w-lg bg-white border-2 border-[#E8E6DC] shadow-[0_16px_64px_rgba(0,0,0,0.08)] rounded-[36px] transition-colors duration-300 ${
          isAdminInput ? 'ring-2 ring-[#febc11] shadow-[0_16px_64px_rgba(254,188,17,0.15)] border-[#febc11]/30' : ''
        }`}>
          <div className="p-12 space-y-9">
            <div className="text-center space-y-4">
              ...
              <h1 className="text-5xl font-semibold text-[#1a1a2e] tracking-tight leading-tight">MatchIQ</h1>
              <h2 className={`text-2xl font-light transition-colors duration-300 leading-tight ${isAdminInput ? 'text-[#febc11]' : 'text-[#4a4a5a]'}`}>
                {headingText}
              </h2>
              <p className="text-[#8B8B9A] text-sm font-light pt-1">{subText}</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2.5">
                ...
                <Input
                  ...
                  placeholder="Введіть ім'я користувача" required disabled={isLoading}
                  className={`h-14 bg-[#F8F8F5] border-2 border-[#E8E6DC] rounded-[20px] text-[#2A2A3A] placeholder:text-[#B0B0BA] transition-all duration-300 text-base font-light hover:border-[#D0CEC4] hover:bg-[#FAFAF8] ${
                    isAdminInput ? 'focus:border-[#febc11] focus:ring-[#febc11]/20 focus:ring-4 focus:bg-white'
              ...
              <div className="space-y-2.5">
                ...
                <Input
                  ...
                  placeholder="Введіть пароль" required disabled={isLoading}
                  className={`h-14 bg-[#F8F8F5] border-2 border-[#E8E6DC] rounded-[20px] text-[#2A2A3A] placeholder:text-[#B0B0BA] transition-all duration-300 text-base font-light hover:border-[#D0CEC4] hover:bg-[#FAFAF8] ${
                    isAdminInput ? 'focus:border-[#febc11] focus:ring-[#febc11]/20 focus:ring-4 focus:bg-white'
                      : 'focus:border-[#3e75ff] focus:ring-[#3e75ff]/15 focus:ring-4 focus:bg-white'
                  }`}
              ...
              <Button type="submit" disabled={isLoading}
                className={`w-full h-14 text-white font-semibold rounded-[20px] transition-all duration-300 ease-out text-base mt-6 transform hover:scale-[1.02] active:scale-[0.98] ${
                  isAdminInput
              ...
              >
                {isLoading ? (
                  <><Loader2 className="mr-2.5 h-5 w-5 animate-spin" strokeWidth={2} />Вхід...</>
                ) : (
                  <><LogIn className="mr-2.5 h-5 w-5" strokeWidth={2} />Увійти</>
                )}
              </Button>
            </form>
            ...
            <div className="text-center space-y-3">
              <p className="text-sm text-[#8B8B9A] font-light">
                {footerLinkText}{' '}
                <a href={footerLinkHref} className="text-[#3e75ff] font-medium hover:text-[#2a5fd4] transition-colors duration-200 hover:underline underline-offset-4">
                  {footerLinkLabel}
                </a>
              </p>
