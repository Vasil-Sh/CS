...
import {
  parseMatchType, determineTier, determineFavorite,
  isMatchFinished, getMatchStatus,
} from '@/lib/csApi';
...
describe('📅 СЦЕНАРІЙ 1: Ідеальний тиждень (+3500₴, 70% winrate)', () => {
  const weeksBets: Bet[] = [
    { match: 'NaVi vs FaZe',      betType: 'П1', odds: 1.80, amount: 1000, date: '2026-06-15', result: 'Win',  profit: 800 },
    { match: 'G2 vs Vitality',    betType: 'П2', odds: 2.10, amount: 500,  date: '2026-06-15', result: 'Loss', profit: -500 },
    { match: 'MOUZ vs Spirit',    betType: 'П1', odds: 2.50, amount: 800,  date: '2026-06-16', result: 'Win',  profit: 1200 },
    { match: 'Astralis vs ENCE',  betType: 'П1', odds: 1.65, amount: 1000, date: '2026-06-17', result: 'Win',  profit: 650 },
    { match: 'Heroic vs BIG',     betType: 'П2', odds: 3.00, amount: 400,  date: '2026-06-17', result: 'Loss', profit: -400 },
    { match: 'Cloud9 vs Liquid',  betType: 'П1', odds: 1.90, amount: 1200, date: '2026-06-18', result: 'Win',  profit: 1080 },
    { match: 'FURIA vs Imperial', betType: 'П1', odds: 1.55, amount: 1500, date: '2026-06-19', result: 'Win',  profit: 825 },
    { match: 'VP vs BetBoom',     betType: 'П2', odds: 2.20, amount: 600,  date: '2026-06-20', result: 'Win',  profit: 720 },
    { match: 'EF vs SAW',         betType: 'П1', odds: 1.75, amount: 900,  date: '2026-06-20', result: 'Loss', profit: -900 },
    { match: '3DMAX vs GL',       betType: 'П1', odds: 2.00, amount: 1000, date: '2026-06-21', result: 'Win',  profit: 1000 },
  ];
  ...
  it('🔍 найприбутковіший матч: MOUZ vs Spirit (+1200₴, odds 2.50)', () => {
    const best = weeksBets.reduce((max, b) => (b.profit ?? 0) > (max.profit ?? 0) ? b : max);
    expect(best.match).toBe('MOUZ vs Spirit');
    expect(best.profit).toBe(1200);
    expect(best.odds).toBe(2.50);
  });

  it('📉 найзбитковіший матч: 3DMAX vs GL (−900₴)', () => {
    const worst = weeksBets.reduce((min, b) => (b.profit ?? 0) < (min.profit ?? Infinity) ? b : min);
    expect(worst.match).toBe('EF vs SAW');
    expect(worst.profit).toBe(-900);
  });

  it('📏 середній коефіцієнт: ~2.05', () => {
    const avgOdds = weeksBets.reduce((sum, b) => sum + b.odds, 0) / weeksBets.length;
...
describe('📉 СЦЕНАРІЙ 2: Чорна смуга (5 поразок поспіль, −5000₴)', () => {
  const badStreak: Bet[] = [
    { match: 'T1 vs T2', betType: 'П1', odds: 2.00, amount: 1000, date: '2026-06-15', result: 'Loss', profit: -1000 },
    { match: 'T3 vs T4', betType: 'П1', odds: 1.80, amount: 1000, date: '2026-06-16', result: 'Loss', profit: -1000 },
    { match: 'T5 vs T6', betType: 'П2', odds: 2.50, amount: 1000, date: '2026-06-17', result: 'Loss', profit: -1000 },
    { match: 'T7 vs T8', betType: 'П1', odds: 1.60, amount: 1000, date: '2026-06-18', result: 'Loss', profit: -1000 },
    { match: 'T9 vs T10', betType: 'П2', odds: 3.00, amount: 1000, date: '2026-06-19', result: 'Loss', profit: -1000 },
  ];
  ...
  it('🔻 найдовша серія поразок = 5', () => {
    ...
    for (const b of badStreak) {
      if (b.result === 'Loss') { streak++; maxStreak = Math.max(maxStreak, streak); }
      else streak = 0;
    }
...
describe('🎯 СЦЕНАРІЙ 3: Експрес-мисливець (ординари + експреси)', () => {
  const expressHunter: Bet[] = [
    { match: 'NaVi vs FaZe',     betType: 'П1',                odds: 1.80, amount: 500,  date: '2026-06-15', result: 'Win',  profit: 400 },
    { match: 'G2 vs Vitality',   betType: 'П2',                odds: 2.50, amount: 300,  date: '2026-06-16', result: 'Loss', profit: -300 },
    { match: 'MOUZ vs Spirit',   betType: 'Експрес 3x | ...',  odds: 4.20, amount: 200,  date: '2026-06-17', result: 'Win',  profit: 640,  format: '3x' },
    { match: 'Heroic vs BIG',    betType: 'П1',                odds: 1.55, amount: 800,  date: '2026-06-18', result: 'Win',  profit: 440 },
    { match: 'VP vs C9',         betType: 'Експрес 2x | ...',  odds: 3.60, amount: 300,  date: '2026-06-19', result: 'Loss', profit: -300, format: '2x' },
    { match: 'Ast vs ENCE',      betType: 'П2',                odds: 2.80, amount: 400,  date: '2026-06-20', result: 'Loss', profit: -400 },
  ];
...
describe('🔄 СЦЕНАРІЙ 4: End-to-end — AI → ставка → результат', () => {
  it('🤖 AI рекомендує NaVi (confidence 75, low risk) → prompt містить команди й формат', () => {
    const prompt = buildPrompt({ team1: 'NaVi', team2: 'FaZe', format: 'BO3', tier: 'tier1' });
  ...
  it('✅ гравець ставить 1000₴ на NaVi @1.80 → Win → +800₴', () => {
    const bet: Bet = {
      match: 'NaVi vs FaZe', betType: 'П1', odds: 1.80, amount: 1000,
      date: '2026-06-18', result: 'Win', profit: 800,
    };
  ...
  it('❌ гравець ставить 500₴ на аутсайдера @3.50 → Loss → −500₴', () => {
    const bet: Bet = {
      match: 'Underdog vs Favorite', betType: 'П2', odds: 3.50, amount: 500,
      date: '2026-06-18', result: 'Loss', profit: -500,
    };
  ...
  it('📊 загальний результат: +800 (NaVi) −500 (Underdog) = +300₴', () => {
    const bets: Bet[] = [
      { match: 'NaVi vs FaZe',          betType: 'П1', odds: 1.80, amount: 1000, date: '2026-06-18', result: 'Win',  profit: 800 },
      { match: 'Underdog vs Favorite',  betType: 'П2', odds: 3.50, amount: 500,  date: '2026-06-18', result: 'Loss', profit: -500 },
    ];
...
describe('🔄 СЦЕНАРІЙ 5: Ростер-аналіз (tier + favorite)', () => {
  ...
  it('📊 BO5 гранд-фінал: 3-2 → finished', () => {
    const match = {
      id: 1, date: '2026-06-18T20:00:00', link: '/gf', type: 'bo5',
      score1: 3, score2: 2, stars: 5,
      nameTeam1: 'G2', nameTeam2: 'FaZe',
      lastChangeDateTeam1: null, lastChangeDateTeam2: null,
      positionTeam1: 5, positionTeam2: 8,
      logoTeam1: null, logoTeam2: null,
      predictionPercentTeam1: 55, predictionPercentTeam2: 45,
      bettingCoefficientTeam1: 1.80, bettingCoefficientTeam2: 2.00,
    };
  ...
  it('📊 BO3 матч 1-1 → ще не finished', () => {
    ...
    const match = {
      id: 2, date: pastDate, link: '/live', type: 'bo3',
      score1: 1, score2: 1, stars: 3,
      nameTeam1: 'A', nameTeam2: 'B',
      lastChangeDateTeam1: null, lastChangeDateTeam2: null,
      positionTeam1: 10, positionTeam2: 12,
      logoTeam1: null, logoTeam2: null,
...
describe('💪 СЦЕНАРІЙ 6: Стрес-тест (1000 ставок)', () => {
  function generateBets(count: number): Bet[] {
    const teams = ['NaVi', 'FaZe', 'G2', 'Vitality', 'MOUZ', 'Spirit', 'Heroic', 'ENCE', 'VP', 'C9'];
    ...
    return Array.from({ length: count }, (_, i) => {
      ...
      return {
        match: `${t1} vs ${t2}`, betType: 'П1', odds,
        amount, date: `2026-06-${String((i % 28) + 1).padStart(2, '0')}`,
        result, profit: result === 'Win' ? amount * (odds - 1) : result === 'Loss' ? -amount : 0,
      };
    });
  }
  ...
  it('1000 Pending → profit = 0', () => {
    const bets = generateBets(1000).map(b => ({ ...b, result: 'Pending' as const, profit: 0 }));
  ...
  it('ручна перевірка: 3 конкретні ставки → сума збігається з ручним розрахунком', () => {
    const manualBets: Bet[] = [
      { match: 'A vs B', betType: 'П1', odds: 2.00, amount: 100, date: '2026-06-18', result: 'Win',  profit: 100 },
      { match: 'C vs D', betType: 'П1', odds: 1.50, amount: 200, date: '2026-06-18', result: 'Win',  profit: 100 },
      { match: 'E vs F', betType: 'П2', odds: 3.00, amount: 150, date: '2026-06-18', result: 'Loss', profit: -150 },
    ];
    // 100 + 100 - 150 = 50
