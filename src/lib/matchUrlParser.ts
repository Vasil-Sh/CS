...
export function parseDota2MatchFromUrl(url: string): ParsedMatchResult | null {
  try {
    // Broader regex: supports dots, uppercase, hyphen, underscore; optional trailing slash
    ...
    // Try to detect tournament keywords in the part after team2
    ...
    let tournamentStart = afterVs.length;
    for (let i = 0; i < afterVs.length; i++) {
      if (
        tournamentKeywords.some((kw) => afterVs[i].toLowerCase().includes(kw))
      ) {
        tournamentStart = i;
        break;
      }
    ...
    const team2Parts = afterVs.slice(0, tournamentStart);
    const tournamentParts = afterVs.slice(tournamentStart);

    const team2 = team2Parts
      .map((p) => p.charAt(0).toUpperCase() + p.slice(1))
      .join(" ");
    const tournament =
      tournamentParts.length > 0
        ? tournamentParts
            .map((p) => p.charAt(0).toUpperCase() + p.slice(1))
            .join(" ")
        : "Dota 2 Tournament";
...
export function parseCS2MatchFromUrl(url: string): ParsedMatchResult | null {
  try {
    ...
    for (let i = 0; i < afterVs.length; i++) {
      if (
        TOURNAMENT_KEYWORDS.some((kw) => afterVs[i].toLowerCase().includes(kw))
      ) {
