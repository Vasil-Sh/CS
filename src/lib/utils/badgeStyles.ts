...
export function getStatusBadge(status: string): string {
  switch (status) {
    ...
    case 'Ризиковані':
      return 'bg-orange-100 text-orange-600 hover:bg-orange-100 border border-orange-200 rounded-full font-medium text-sm px-3 py-1';
    case 'Під питанням':
      return 'bg-amber-100 text-amber-600 hover:bg-amber-100 border border-amber-200 rounded-full font-medium text-sm px-3 py-1';
    case 'Стабільні':
